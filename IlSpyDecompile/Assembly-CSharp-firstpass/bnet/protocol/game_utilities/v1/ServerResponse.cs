...
namespace bnet.protocol.game_utilities.v1
{
	public class ServerResponse : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (Attribute.Count != serverResponse.Attribute.Count)
			{
				return false;
			}
			for (int i = 0; i < Attribute.Count; i++)
			{
				if (!Attribute[i].Equals(serverResponse.Attribute[i]))
				{
					return false;
				}
			}
		...
		{
			return ProtobufUtil.ParseFrom<ServerResponse>(bs);
		}
		...
		public static ServerResponse DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, serverResponse);
