...
namespace bnet.protocol.broadcast.v1
{
	public class BroadcastNotification : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (HasBroadcast != broadcastNotification.HasBroadcast || (HasBroadcast && !Broadcast.Equals(broadcastNotification.Broadcast)))
			{
				return false;
			}
			if (HasId != broadcastNotification.HasId || (HasId && !Id.Equals(broadcastNotification.Id)))
			{
				return false;
			}
			return true;
		...
		{
			return ProtobufUtil.ParseFrom<BroadcastNotification>(bs);
		}
		...
		public static BroadcastNotification DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, broadcastNotification);
