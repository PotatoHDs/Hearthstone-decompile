...
namespace bnet.protocol.matchmaking.v1
{
	public class AddPlayersResponse : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (ConnectInfo.Count != addPlayersResponse.ConnectInfo.Count)
			{
				return false;
			}
			for (int i = 0; i < ConnectInfo.Count; i++)
			{
				if (!ConnectInfo[i].Equals(addPlayersResponse.ConnectInfo[i]))
				{
					return false;
				}
			}
		...
		{
			return ProtobufUtil.ParseFrom<AddPlayersResponse>(bs);
		}
		...
		public static AddPlayersResponse DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, addPlayersResponse);
		...
		public static void Serialize(Stream stream, AddPlayersResponse instance)
		{
			...
			foreach (ConnectInfo item in instance.ConnectInfo)
			{
				...
				bnet.protocol.matchmaking.v1.ConnectInfo.Serialize(stream, item);
			}
		}
