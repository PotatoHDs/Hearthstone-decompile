...
namespace bnet.protocol.matchmaking.v1
{
	public class GameMatchmakerFilter : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (Attribute.Count != gameMatchmakerFilter.Attribute.Count)
			{
				return false;
			}
			for (int i = 0; i < Attribute.Count; i++)
			{
				if (!Attribute[i].Equals(gameMatchmakerFilter.Attribute[i]))
				{
					return false;
				}
			}
		...
		{
			return ProtobufUtil.ParseFrom<GameMatchmakerFilter>(bs);
		}
		...
		public static GameMatchmakerFilter DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, gameMatchmakerFilter);
