...
namespace bnet.protocol.matchmaking.v1
{
	public class GetMatchmakingEntriesRequest : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (getMatchmakingEntriesRequest == null)
			{
				return false;
			}
			if (HasMatchmakerId != getMatchmakingEntriesRequest.HasMatchmakerId || (HasMatchmakerId && !MatchmakerId.Equals(getMatchmakingEntriesRequest.MatchmakerId)))
			{
				return false;
			}
			if (HasNumRequests != getMatchmakingEntriesRequest.HasNumRequests || (HasNumRequests && !NumRequests.Equals(getMatchmakingEntriesRequest.NumRequests)))
			{
				return false;
			}
			if (HasMatchmakerGuid != getMatchmakingEntriesRequest.HasMatchmakerGuid || (HasMatchmakerGuid && !MatchmakerGuid.Equals(getMatchmakingEntriesRequest.MatchmakerGuid)))
			{
				return false;
			}
			return true;
		...
		{
			return ProtobufUtil.ParseFrom<GetMatchmakingEntriesRequest>(bs);
		}
		...
		public static GetMatchmakingEntriesRequest DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, getMatchmakingEntriesRequest);
		...
		public uint GetSerializedSize()
		{
			...
			{
				num++;
				num += ProtocolParser.SizeOfUInt32(NumRequests);
			}
			if (HasMatchmakerGuid)
			{
				num++;
