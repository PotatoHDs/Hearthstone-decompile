...
namespace bnet.protocol.user_manager.v1
{
	public class RecentPlayersAddedNotification : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (Player.Count != recentPlayersAddedNotification.Player.Count)
			{
				return false;
			}
			for (int i = 0; i < Player.Count; i++)
			{
				if (!Player[i].Equals(recentPlayersAddedNotification.Player[i]))
				{
					return false;
				}
			}
		...
		{
			return ProtobufUtil.ParseFrom<RecentPlayersAddedNotification>(bs);
		}
		...
		public static RecentPlayersAddedNotification DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, recentPlayersAddedNotification);
