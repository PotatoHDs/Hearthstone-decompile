...
namespace bnet.protocol.channel.v1
{
	public class LeaveNotification : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (HasAgentId != leaveNotification.HasAgentId || (HasAgentId && !AgentId.Equals(leaveNotification.AgentId)))
			{
				return false;
			}
			if (!MemberId.Equals(leaveNotification.MemberId))
			{
				return false;
			}
			if (HasReason != leaveNotification.HasReason || (HasReason && !Reason.Equals(leaveNotification.Reason)))
			{
				return false;
			}
			if (HasChannelId != leaveNotification.HasChannelId || (HasChannelId && !ChannelId.Equals(leaveNotification.ChannelId)))
			{
				return false;
			}
			if (HasSubscriberId != leaveNotification.HasSubscriberId || (HasSubscriberId && !SubscriberId.Equals(leaveNotification.SubscriberId)))
			{
				return false;
			}
			return true;
		...
		{
			return ProtobufUtil.ParseFrom<LeaveNotification>(bs);
		}
		...
		public static LeaveNotification DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, leaveNotification);
