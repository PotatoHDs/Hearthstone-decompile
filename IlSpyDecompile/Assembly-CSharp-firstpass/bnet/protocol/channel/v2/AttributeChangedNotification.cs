...
namespace bnet.protocol.channel.v2
{
	public class AttributeChangedNotification : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (HasAgentId != attributeChangedNotification.HasAgentId || (HasAgentId && !AgentId.Equals(attributeChangedNotification.AgentId)))
			{
				return false;
			}
			if (HasSubscriberId != attributeChangedNotification.HasSubscriberId || (HasSubscriberId && !SubscriberId.Equals(attributeChangedNotification.SubscriberId)))
			{
				return false;
			}
			if (HasChannelId != attributeChangedNotification.HasChannelId || (HasChannelId && !ChannelId.Equals(attributeChangedNotification.ChannelId)))
			{
				return false;
			}
			if (Attribute.Count != attributeChangedNotification.Attribute.Count)
			{
				return false;
			}
			for (int i = 0; i < Attribute.Count; i++)
			{
				if (!Attribute[i].Equals(attributeChangedNotification.Attribute[i]))
				{
					return false;
				}
			}
		...
		{
			return ProtobufUtil.ParseFrom<AttributeChangedNotification>(bs);
		}
		...
		public static AttributeChangedNotification DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, attributeChangedNotification);
