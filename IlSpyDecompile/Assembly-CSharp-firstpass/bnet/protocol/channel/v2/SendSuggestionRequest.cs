...
namespace bnet.protocol.channel.v2
{
	public class SendSuggestionRequest : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (sendSuggestionRequest == null)
			{
				return false;
			}
			if (HasAgentId != sendSuggestionRequest.HasAgentId || (HasAgentId && !AgentId.Equals(sendSuggestionRequest.AgentId)))
			{
				return false;
			}
			if (HasOptions != sendSuggestionRequest.HasOptions || (HasOptions && !Options.Equals(sendSuggestionRequest.Options)))
			{
				return false;
			}
			return true;
		...
		{
			return ProtobufUtil.ParseFrom<SendSuggestionRequest>(bs);
		}
		...
		public static SendSuggestionRequest DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, sendSuggestionRequest);
