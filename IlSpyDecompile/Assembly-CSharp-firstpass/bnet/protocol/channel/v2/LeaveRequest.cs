...
namespace bnet.protocol.channel.v2
{
	public class LeaveRequest : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (HasAgentId != leaveRequest.HasAgentId || (HasAgentId && !AgentId.Equals(leaveRequest.AgentId)))
			{
				return false;
			}
			if (HasChannelId != leaveRequest.HasChannelId || (HasChannelId && !ChannelId.Equals(leaveRequest.ChannelId)))
			{
				return false;
			}
			return true;
		...
		{
			return ProtobufUtil.ParseFrom<LeaveRequest>(bs);
		}
		...
		public static LeaveRequest DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, leaveRequest);
			return leaveRequest;
		}
