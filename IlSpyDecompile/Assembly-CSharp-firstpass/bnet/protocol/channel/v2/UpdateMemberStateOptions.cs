...
namespace bnet.protocol.channel.v2
{
	public class UpdateMemberStateOptions : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (HasMemberId != updateMemberStateOptions.HasMemberId || (HasMemberId && !MemberId.Equals(updateMemberStateOptions.MemberId)))
			{
				return false;
			}
			if (Attribute.Count != updateMemberStateOptions.Attribute.Count)
			{
				return false;
			}
			for (int i = 0; i < Attribute.Count; i++)
			{
				if (!Attribute[i].Equals(updateMemberStateOptions.Attribute[i]))
				{
					return false;
				}
			}
		...
		{
			return ProtobufUtil.ParseFrom<UpdateMemberStateOptions>(bs);
		}
		...
		public static UpdateMemberStateOptions DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, updateMemberStateOptions);
