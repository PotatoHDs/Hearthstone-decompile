...
namespace bnet.protocol.friends.v2.client
{
	public class ReceivedInvitationRemovedNotification : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (HasAgentAccountId != receivedInvitationRemovedNotification.HasAgentAccountId || (HasAgentAccountId && !AgentAccountId.Equals(receivedInvitationRemovedNotification.AgentAccountId)))
			{
				return false;
			}
			if (Assignment.Count != receivedInvitationRemovedNotification.Assignment.Count)
			{
				return false;
			}
			for (int i = 0; i < Assignment.Count; i++)
			{
				if (!Assignment[i].Equals(receivedInvitationRemovedNotification.Assignment[i]))
				{
					return false;
				}
			}
