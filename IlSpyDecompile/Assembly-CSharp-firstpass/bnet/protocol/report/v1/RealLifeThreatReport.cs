...
namespace bnet.protocol.report.v1
{
	public class RealLifeThreatReport : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (HasTarget != realLifeThreatReport.HasTarget || (HasTarget && !Target.Equals(realLifeThreatReport.Target)))
			{
				return false;
			}
			if (HasText != realLifeThreatReport.HasText || (HasText && !Text.Equals(realLifeThreatReport.Text)))
			{
				return false;
			}
			return true;
		...
		{
			return ProtobufUtil.ParseFrom<RealLifeThreatReport>(bs);
		}
		...
		public static RealLifeThreatReport DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, realLifeThreatReport);
