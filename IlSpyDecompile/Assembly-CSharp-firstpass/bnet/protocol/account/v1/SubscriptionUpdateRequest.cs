...
namespace bnet.protocol.account.v1
{
	public class SubscriptionUpdateRequest : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (Ref.Count != subscriptionUpdateRequest.Ref.Count)
			{
				return false;
			}
			for (int i = 0; i < Ref.Count; i++)
			{
				if (!Ref[i].Equals(subscriptionUpdateRequest.Ref[i]))
				{
					return false;
				}
			}
		...
		{
			return ProtobufUtil.ParseFrom<SubscriptionUpdateRequest>(bs);
		}
		...
		public static SubscriptionUpdateRequest DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, subscriptionUpdateRequest);
