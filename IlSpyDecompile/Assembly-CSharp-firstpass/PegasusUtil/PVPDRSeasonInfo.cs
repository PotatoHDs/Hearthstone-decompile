...
namespace PegasusUtil
{
	public class PVPDRSeasonInfo : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (HasSeason != pVPDRSeasonInfo.HasSeason || (HasSeason && !Season.Equals(pVPDRSeasonInfo.Season)))
			{
				return false;
			}
			if (HasSeasonEndingSoonDays != pVPDRSeasonInfo.HasSeasonEndingSoonDays || (HasSeasonEndingSoonDays && !SeasonEndingSoonDays.Equals(pVPDRSeasonInfo.SeasonEndingSoonDays)))
			{
				return false;
			}
			if (HasSeasonEndingSoonPrefab != pVPDRSeasonInfo.HasSeasonEndingSoonPrefab || (HasSeasonEndingSoonPrefab && !SeasonEndingSoonPrefab.Equals(pVPDRSeasonInfo.SeasonEndingSoonPrefab)))
			{
				return false;
			}
			if (HasSeasonEndingSoonPrefabExtra != pVPDRSeasonInfo.HasSeasonEndingSoonPrefabExtra || (HasSeasonEndingSoonPrefabExtra && !SeasonEndingSoonPrefabExtra.Equals(pVPDRSeasonInfo.SeasonEndingSoonPrefabExtra)))
			{
				return false;
			}
			if (HasNextStartSecondsFromNow != pVPDRSeasonInfo.HasNextStartSecondsFromNow || (HasNextStartSecondsFromNow && !NextStartSecondsFromNow.Equals(pVPDRSeasonInfo.NextStartSecondsFromNow)))
			{
				return false;
			}
			if (HasNextSeasonId != pVPDRSeasonInfo.HasNextSeasonId || (HasNextSeasonId && !NextSeasonId.Equals(pVPDRSeasonInfo.NextSeasonId)))
			{
				return false;
			}
			if (HasNextSeasonComingSoonDays != pVPDRSeasonInfo.HasNextSeasonComingSoonDays || (HasNextSeasonComingSoonDays && !NextSeasonComingSoonDays.Equals(pVPDRSeasonInfo.NextSeasonComingSoonDays)))
			{
				return false;
			}
			if (HasNextSeasonComingSoonPrefab != pVPDRSeasonInfo.HasNextSeasonComingSoonPrefab || (HasNextSeasonComingSoonPrefab && !NextSeasonComingSoonPrefab.Equals(pVPDRSeasonInfo.NextSeasonComingSoonPrefab)))
			{
				return false;
			}
			if (HasNextSeasonComingSoonPrefabExtra != pVPDRSeasonInfo.HasNextSeasonComingSoonPrefabExtra || (HasNextSeasonComingSoonPrefabExtra && !NextSeasonComingSoonPrefabExtra.Equals(pVPDRSeasonInfo.NextSeasonComingSoonPrefabExtra)))
			{
				return false;
			}
			if (NextSeasonStrings.Count != pVPDRSeasonInfo.NextSeasonStrings.Count)
			{
				return false;
			}
			for (int i = 0; i < NextSeasonStrings.Count; i++)
			{
				if (!NextSeasonStrings[i].Equals(pVPDRSeasonInfo.NextSeasonStrings[i]))
				{
					return false;
				}
			}
		...
		public static PVPDRSeasonInfo DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, pVPDRSeasonInfo);
