...
public class DRGA_Evil_Fight_11 : DRGA_Dungeon
{
	...
	protected override void PlayEmoteResponse(EmoteType emoteType, CardSoundSpell emoteSpell)
	{
		...
		if (emoteType == EmoteType.START)
		{
			if (!m_Heroic)
			{
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_01h_Male_Human_Evil_Fight_11_BossStart_Reno_01, Notification.SpeechBubbleDirection.TopRight, actor));
				return;
			}
		...
		if (MissionEntity.STANDARD_EMOTE_RESPONSE_TRIGGERS.Contains(emoteType))
		{
			...
			{
			case "DRGA_BOSS_01h4":
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_01h_Male_Human_Evil_Fight_11_EmoteResponse_Reno_01, Notification.SpeechBubbleDirection.TopRight, actor));
				break;
			case "DRGA_BOSS_37h":
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_37h_Male_Human_Evil_Fight_11_EmoteResponse_Anduin_01, Notification.SpeechBubbleDirection.TopRight, actor));
				break;
			case "DRGA_BOSS_38h":
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_38h_Female_Forsaken_Evil_Fight_11_EmoteResponse_Sylvanas_01, Notification.SpeechBubbleDirection.TopRight, actor));
				break;
			default:
				Gameplay.Get().StartCoroutine(PlaySoundAndBlockSpeech(VO_DRGA_BOSS_01h_Male_Human_Evil_Fight_11_EmoteResponse_Reno_01, Notification.SpeechBubbleDirection.TopRight, actor));
				break;
	...
	protected override IEnumerator HandleMissionEventWithTiming(int missionEvent)
	{
		...
		case 250:
		{
			int num = GetTag(GAME_TAG.TURN) - GameState.Get().GetGameEntity().GetTag(GAME_TAG.EXTRA_TURNS_TAKEN_THIS_GAME);
			if (m_TurnOfGalakrondLine >= num)
			{
				break;
			}
			m_TurnOfGalakrondLine = num;
	...
	public override void OnPlayThinkEmote()
	{
		...
		if (thinkEmoteBossThinkChancePercentage > num)
		{
			...
			case "DRGA_BOSS_01h4":
			{
				string line2 = PopRandomLine(m_RenoIdleLinesCopy);
			...
			case "DRGA_BOSS_37h":
			{
				string line3 = PopRandomLine(m_AnduinIdleLinesCopy);
				if (m_AnduinIdleLinesCopy.Count == 0)
				{
					m_AnduinIdleLinesCopy = new List<string>(m_AnduinIdleLines);
				}
				Gameplay.Get().StartCoroutine(PlayBossLine(actor, line3));
				break;
			}
			case "DRGA_BOSS_38h":
			{
				string line = PopRandomLine(m_SylvanasIdleLinesCopy);
