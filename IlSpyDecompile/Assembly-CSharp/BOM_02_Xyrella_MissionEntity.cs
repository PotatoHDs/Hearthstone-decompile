...
public abstract class BOM_02_Xyrella_MissionEntity : GenericDungeonMissionEntity
{
	...
	protected virtual IEnumerator OnBossHeroPowerPlayed(Entity entity)
	{
		...
		if (!m_enemySpeaking && !m_MissionDisableAutomaticVO && !(num < num2) && !(GameState.Get().GetOpposingSidePlayer().GetHero()
			.GetCard()
			.GetActor() == null))
		{
	...
	public override IEnumerator OnPlayThinkEmoteWithTiming()
	{
		...
		else if (m_Mission_FriendlyPlayIdleLines)
		{
			...
			{
				yield return MissionPlayThinkEmote(actor2);
			}
			else
			{
	...
	protected IEnumerator MissionPlayThinkEmote(Actor thinkingActor)
	{
		...
		{
			CardSoundSpell cardSoundSpell = GameState.Get().GetOpposingSidePlayer().GetHeroCard()
				.PlayEmote(emoteType);
			AudioSource activeAudioSource2 = cardSoundSpell.GetActiveAudioSource();
			yield return GameState.Get().GetOpposingSidePlayer().GetHeroCard()
				.PlayEmote(emoteType);
			yield return new WaitForSeconds(activeAudioSource2.clip.length);
		}
		else if (thinkingActor == actor2)
		{
			CardSoundSpell cardSoundSpell2 = GameState.Get().GetOpposingSidePlayer().GetHeroCard()
				.PlayEmote(emoteType);
			AudioSource activeAudioSource2 = cardSoundSpell2.GetActiveAudioSource();
			yield return GameState.Get().GetOpposingSidePlayer().GetHeroCard()
				.PlayEmote(emoteType);
			yield return new WaitForSeconds(activeAudioSource2.clip.length);
		}
	...
	public bool shouldPlayBanterVO()
	{
		...
		float num = m_lastVOplayFinshtime + m_BanterVOSilenceTime;
		if (time > num)
		{
			return true;
		}
	...
	protected IEnumerator MissionPlayVO(AssetReference brassRing, List<string> lines, ShouldPlay shouldPlay, bool bUseBubble = true, bool bPlayOrder = true)
	{
		...
		yield return MissionPlayVO(line: (!bPlayOrder) ? PopRandomLine(lines, removeLine) : PopNextLine(lines, removeLine), brassRing: brassRing, bUseBubble: bUseBubble, shouldPlay: shouldPlay);
	}
	...
	{
		Actor actorByCardId = GetActorByCardId(minionSpeaker);
		...
		{
			yield return MissionPlayVO(actorByCardId, lines, base.InternalShouldPlayOnlyOnce, bUseBubble: false);
		}
	}
	...
	{
		string line = PopRandomLine(lines);
		yield return MissionPlaySound(line);
	}
