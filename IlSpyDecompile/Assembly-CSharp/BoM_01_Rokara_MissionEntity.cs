...
public abstract class BoM_01_Rokara_MissionEntity : GenericDungeonMissionEntity
{
	...
	protected IEnumerator MissionPlayVO(Actor actor, string line, bool bUseBubble, ShouldPlay shouldPlay)
	{
		if (!(actor == null) && line != null)
		{
			...
			{
				yield return GameEntity.Coroutines.StartCoroutine(PlaySoundAndBlockSpeech(line, speakerDirection, actor, 2.5f, 1f, parentBubbleToActor));
			}
			else if (shouldPlay() == InternalShouldPlayOnlyOnce())
			{
				yield return GameEntity.Coroutines.StartCoroutine(PlaySoundAndBlockSpeechOnce(line, speakerDirection, actor, 2.5f, 1f, parentBubbleToActor));
