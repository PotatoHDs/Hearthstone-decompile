...
namespace Blizzard.Telemetry.WTCG.NGDP
{
	public class NoWifi : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (HasUpdatedVersion != noWifi.HasUpdatedVersion || (HasUpdatedVersion && !UpdatedVersion.Equals(noWifi.UpdatedVersion)))
			{
				return false;
			}
			if (HasAvailableSpaceMB != noWifi.HasAvailableSpaceMB || (HasAvailableSpaceMB && !AvailableSpaceMB.Equals(noWifi.AvailableSpaceMB)))
			{
				return false;
			}
			if (HasElapsedSeconds != noWifi.HasElapsedSeconds || (HasElapsedSeconds && !ElapsedSeconds.Equals(noWifi.ElapsedSeconds)))
			{
				return false;
			}
			return true;
		...
		public static NoWifi DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, noWifi);
			return noWifi;
		}
