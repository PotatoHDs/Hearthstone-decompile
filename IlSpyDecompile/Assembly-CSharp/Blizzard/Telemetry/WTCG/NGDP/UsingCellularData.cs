...
namespace Blizzard.Telemetry.WTCG.NGDP
{
	public class UsingCellularData : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (usingCellularData == null)
			{
				return false;
			}
			if (HasUpdatedVersion != usingCellularData.HasUpdatedVersion || (HasUpdatedVersion && !UpdatedVersion.Equals(usingCellularData.UpdatedVersion)))
			{
				return false;
			}
			if (HasAvailableSpaceMB != usingCellularData.HasAvailableSpaceMB || (HasAvailableSpaceMB && !AvailableSpaceMB.Equals(usingCellularData.AvailableSpaceMB)))
			{
				return false;
			}
			if (HasElapsedSeconds != usingCellularData.HasElapsedSeconds || (HasElapsedSeconds && !ElapsedSeconds.Equals(usingCellularData.ElapsedSeconds)))
			{
				return false;
			}
			return true;
		...
		public static UsingCellularData DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, usingCellularData);
