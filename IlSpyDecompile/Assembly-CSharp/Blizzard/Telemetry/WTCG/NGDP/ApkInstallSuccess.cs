...
namespace Blizzard.Telemetry.WTCG.NGDP
{
	public class ApkInstallSuccess : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (apkInstallSuccess == null)
			{
				return false;
			}
			if (HasUpdatedVersion != apkInstallSuccess.HasUpdatedVersion || (HasUpdatedVersion && !UpdatedVersion.Equals(apkInstallSuccess.UpdatedVersion)))
			{
				return false;
			}
			if (HasAvailableSpaceMB != apkInstallSuccess.HasAvailableSpaceMB || (HasAvailableSpaceMB && !AvailableSpaceMB.Equals(apkInstallSuccess.AvailableSpaceMB)))
			{
				return false;
			}
			if (HasElapsedSeconds != apkInstallSuccess.HasElapsedSeconds || (HasElapsedSeconds && !ElapsedSeconds.Equals(apkInstallSuccess.ElapsedSeconds)))
			{
				return false;
			}
			return true;
		...
		public static ApkInstallSuccess DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, apkInstallSuccess);
