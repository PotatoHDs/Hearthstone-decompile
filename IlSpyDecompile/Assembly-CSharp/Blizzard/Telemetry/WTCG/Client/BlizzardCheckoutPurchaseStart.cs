...
namespace Blizzard.Telemetry.WTCG.Client
{
	public class BlizzardCheckoutPurchaseStart : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (blizzardCheckoutPurchaseStart == null)
			{
				return false;
			}
			if (HasPlayer != blizzardCheckoutPurchaseStart.HasPlayer || (HasPlayer && !Player.Equals(blizzardCheckoutPurchaseStart.Player)))
			{
				return false;
			}
			if (HasDeviceInfo != blizzardCheckoutPurchaseStart.HasDeviceInfo || (HasDeviceInfo && !DeviceInfo.Equals(blizzardCheckoutPurchaseStart.DeviceInfo)))
			{
				return false;
			}
			if (HasTransactionId != blizzardCheckoutPurchaseStart.HasTransactionId || (HasTransactionId && !TransactionId.Equals(blizzardCheckoutPurchaseStart.TransactionId)))
			{
				return false;
			}
			if (HasProductId != blizzardCheckoutPurchaseStart.HasProductId || (HasProductId && !ProductId.Equals(blizzardCheckoutPurchaseStart.ProductId)))
			{
				return false;
			}
			if (HasCurrency != blizzardCheckoutPurchaseStart.HasCurrency || (HasCurrency && !Currency.Equals(blizzardCheckoutPurchaseStart.Currency)))
			{
				return false;
			}
			return true;
		...
		public static BlizzardCheckoutPurchaseStart DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, blizzardCheckoutPurchaseStart);
