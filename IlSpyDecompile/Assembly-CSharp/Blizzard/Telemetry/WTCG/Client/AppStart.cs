...
namespace Blizzard.Telemetry.WTCG.Client
{
	public class AppStart : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (HasTestType != appStart.HasTestType || (HasTestType && !TestType.Equals(appStart.TestType)))
			{
				return false;
			}
			if (HasDuration != appStart.HasDuration || (HasDuration && !Duration.Equals(appStart.Duration)))
			{
				return false;
			}
			if (HasClientChangelist != appStart.HasClientChangelist || (HasClientChangelist && !ClientChangelist.Equals(appStart.ClientChangelist)))
			{
				return false;
			}
			return true;
		...
		public static AppStart DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, appStart);
			return appStart;
		}
