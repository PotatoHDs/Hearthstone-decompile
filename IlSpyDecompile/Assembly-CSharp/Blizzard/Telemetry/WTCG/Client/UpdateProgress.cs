...
namespace Blizzard.Telemetry.WTCG.Client
{
	public class UpdateProgress : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (HasDeviceInfo != updateProgress.HasDeviceInfo || (HasDeviceInfo && !DeviceInfo.Equals(updateProgress.DeviceInfo)))
			{
				return false;
			}
			if (HasDuration != updateProgress.HasDuration || (HasDuration && !Duration.Equals(updateProgress.Duration)))
			{
				return false;
			}
			if (HasRealDownloadBytes != updateProgress.HasRealDownloadBytes || (HasRealDownloadBytes && !RealDownloadBytes.Equals(updateProgress.RealDownloadBytes)))
			{
				return false;
			}
			if (HasExpectedDownloadBytes != updateProgress.HasExpectedDownloadBytes || (HasExpectedDownloadBytes && !ExpectedDownloadBytes.Equals(updateProgress.ExpectedDownloadBytes)))
			{
				return false;
			}
			return true;
		...
		public static UpdateProgress DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, updateProgress);
