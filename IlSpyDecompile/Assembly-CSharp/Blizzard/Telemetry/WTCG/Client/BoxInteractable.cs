...
namespace Blizzard.Telemetry.WTCG.Client
{
	public class BoxInteractable : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (HasTestType != boxInteractable.HasTestType || (HasTestType && !TestType.Equals(boxInteractable.TestType)))
			{
				return false;
			}
			if (HasDuration != boxInteractable.HasDuration || (HasDuration && !Duration.Equals(boxInteractable.Duration)))
			{
				return false;
			}
			if (HasClientChangelist != boxInteractable.HasClientChangelist || (HasClientChangelist && !ClientChangelist.Equals(boxInteractable.ClientChangelist)))
			{
				return false;
			}
			return true;
		...
		public static BoxInteractable DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, boxInteractable);
