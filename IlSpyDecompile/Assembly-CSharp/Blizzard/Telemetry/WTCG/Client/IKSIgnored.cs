...
namespace Blizzard.Telemetry.WTCG.Client
{
	public class IKSIgnored : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (HasPlayer != iKSIgnored.HasPlayer || (HasPlayer && !Player.Equals(iKSIgnored.Player)))
			{
				return false;
			}
			if (HasDeviceInfo != iKSIgnored.HasDeviceInfo || (HasDeviceInfo && !DeviceInfo.Equals(iKSIgnored.DeviceInfo)))
			{
				return false;
			}
			if (HasIksCampaignName != iKSIgnored.HasIksCampaignName || (HasIksCampaignName && !IksCampaignName.Equals(iKSIgnored.IksCampaignName)))
			{
				return false;
			}
			if (HasIksMediaUrl != iKSIgnored.HasIksMediaUrl || (HasIksMediaUrl && !IksMediaUrl.Equals(iKSIgnored.IksMediaUrl)))
			{
				return false;
			}
			return true;
		...
		public static IKSIgnored DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, iKSIgnored);
			return iKSIgnored;
		}
