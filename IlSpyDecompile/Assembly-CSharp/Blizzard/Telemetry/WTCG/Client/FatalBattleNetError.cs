...
namespace Blizzard.Telemetry.WTCG.Client
{
	public class FatalBattleNetError : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (fatalBattleNetError == null)
			{
				return false;
			}
			if (HasDeviceInfo != fatalBattleNetError.HasDeviceInfo || (HasDeviceInfo && !DeviceInfo.Equals(fatalBattleNetError.DeviceInfo)))
			{
				return false;
			}
			if (HasErrorCode != fatalBattleNetError.HasErrorCode || (HasErrorCode && !ErrorCode.Equals(fatalBattleNetError.ErrorCode)))
			{
				return false;
			}
			if (HasDescription != fatalBattleNetError.HasDescription || (HasDescription && !Description.Equals(fatalBattleNetError.Description)))
			{
				return false;
			}
			return true;
		...
		public static FatalBattleNetError DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, fatalBattleNetError);
