...
namespace Blizzard.Telemetry.WTCG.Client
{
	public class MasterVolumeChanged : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (HasPlayer != masterVolumeChanged.HasPlayer || (HasPlayer && !Player.Equals(masterVolumeChanged.Player)))
			{
				return false;
			}
			if (HasDeviceInfo != masterVolumeChanged.HasDeviceInfo || (HasDeviceInfo && !DeviceInfo.Equals(masterVolumeChanged.DeviceInfo)))
			{
				return false;
			}
			if (HasOldVolume != masterVolumeChanged.HasOldVolume || (HasOldVolume && !OldVolume.Equals(masterVolumeChanged.OldVolume)))
			{
				return false;
			}
			if (HasNewVolume != masterVolumeChanged.HasNewVolume || (HasNewVolume && !NewVolume.Equals(masterVolumeChanged.NewVolume)))
			{
				return false;
			}
			return true;
		...
		public static MasterVolumeChanged DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, masterVolumeChanged);
