...
namespace Blizzard.Telemetry.WTCG.Client
{
	public class AccountHealUpResult : IProtoBuf
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (accountHealUpResult == null)
			{
				return false;
			}
			if (HasPlayer != accountHealUpResult.HasPlayer || (HasPlayer && !Player.Equals(accountHealUpResult.Player)))
			{
				return false;
			}
			if (HasDeviceInfo != accountHealUpResult.HasDeviceInfo || (HasDeviceInfo && !DeviceInfo.Equals(accountHealUpResult.DeviceInfo)))
			{
				return false;
			}
			if (HasResult != accountHealUpResult.HasResult || (HasResult && !Result.Equals(accountHealUpResult.Result)))
			{
				return false;
			}
			if (HasErrorCode != accountHealUpResult.HasErrorCode || (HasErrorCode && !ErrorCode.Equals(accountHealUpResult.ErrorCode)))
			{
				return false;
			}
			return true;
		...
		public static AccountHealUpResult DeserializeLengthDelimited(Stream stream)
		{
			...
			DeserializeLengthDelimited(stream, accountHealUpResult);
